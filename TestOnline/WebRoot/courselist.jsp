...
<body>
    ...
    <input type="hidden" name="action" value="findcourse"/>
    <ul class="toolbar">
    	
        <li> 专业： </li>
        <li><select id="depart" name="depart" class="find">
							<option value="computer01">按课程号</option>
							<option value="computer02">按课程名</option>
    ...
    <form action="${pageContext.request.contextPath}/SecServlet"
												 method="post">
    <input type="hidden" name="action" value="exportcourse"/>
    
    <ul class="toolbar">
    	  
       <li><input type="submit" value="导出所有课程" class="btn"></li></ul>
    ...
    <p><a href="${pageContext.request.contextPath}/SecServlet?action=findcourse&id=${n.id}&key=course">编辑</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:if(confirm('确定要删除吗？')) location='${pageContext.request.contextPath}/SecServlet?action=deletecourse&id=${n.id}&key=course'">删除</a></p>
    </li>
    </c:forEach>
